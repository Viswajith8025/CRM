-- ==============================================================================
-- LEAD <-> CLIENT BIDIRECTIONAL SYNC ENGINE
-- ==============================================================================
-- Requirement: 
-- 1) If a Lead is marked as 'active_client' (or 'converted'), they must automatically appear in Clients.
-- 2) If a Client is added directly (e.g. via Onboarding form), they must automatically appear in Leads as 'active_client'.
-- ==============================================================================

-- 1. Sync: Leads -> Clients
...
-- We use a BEFORE trigger to assign NEW.lead_id without needing a recursive UPDATE
DROP TRIGGER IF EXISTS trg_sync_client_to_lead ON public.clients;
CREATE TRIGGER trg_sync_client_to_lead
  BEFORE INSERT ON public.clients
  FOR EACH ROW
  EXECUTE FUNCTION public.sync_client_to_lead();
