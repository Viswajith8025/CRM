...
export function CriticalDeadlinesWidget() {
  const { criticalDeadlines, fetchCriticalDeadlines } = useDashboardDataStore()

  useEffect(() => {
    fetchCriticalDeadlines()
  }, [])
              ...
              return (
                <div key={task.id} className="flex items-center justify-between p-3 rounded-lg bg-white/5 border border-white/5 hover:border-rose-500/30 transition-colors">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[10px] text-white/30 font-bold uppercase tracking-tight truncate">{task.project_name || 'General'}</p>
                  </div>
                  <div className={cn(
                    "flex flex-col items-end px-2 py-1 rounded",
                    task.is_overdue ? "bg-rose-500/10" : "bg-amber-500/10"
                  )}>
                    <span className={cn(
                      "text-[10px] font-black uppercase tracking-tighter",
                      task.is_overdue ? "text-rose-400" : "text-amber-400"
                    )}>
                      {task.is_overdue ? "OVERDUE" : format(new Date(task.due_date), 'MMM d')}
                    </span>
                  </div>
                </div>
              )
